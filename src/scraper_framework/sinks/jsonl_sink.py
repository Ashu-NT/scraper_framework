import json
from pathlib import Path
from typing import List

from scraper_framework.core.models import Record, ScrapeJob
from scraper_framework.sinks.base import Sink
from scraper_framework.utils.logging import get_logger


class JsonlSink(Sink):
    """Sink that writes records to a JSONL (JSON Lines) file."""

    def __init__(self):
        self._stream_initialized = False
        self._stream_path = None
        self.log = get_logger("scraper_framework.sink.jsonl")

    def write(self, job: ScrapeJob, records: List[Record]) -> None:
        """Write records to JSONL using the job's field_schema for consistent output."""
        path = job.sink_config.get("path", "output.jsonl")
        write_mode = str(job.sink_config.get("write_mode", "overwrite")).lower()
        execution_mode = str(getattr(job, "execution_mode", "memory")).lower()

        self._ensure_parent_dir(path)

        if write_mode not in {"overwrite", "append"}:
            raise ValueError("jsonl sink write_mode must be 'overwrite' or 'append'")

        file_mode = self._resolve_file_mode(path, write_mode, execution_mode)

        with open(path, file_mode, encoding="utf-8") as f:
            for r in records:
                record_dict = {"id": r.id, "source_url": r.source_url, "scraped_at_utc": r.scraped_at_utc}
                record_dict.update(r.fields)
                f.write(json.dumps(record_dict, ensure_ascii=False) + "\n")

        self.log.info(
            "JSONL write: path=%s rows=%d execution_mode=%s write_mode=%s file_mode=%s",
            path,
            len(records),
            execution_mode,
            write_mode,
            file_mode,
        )

    def _resolve_file_mode(self, path: str, write_mode: str, execution_mode: str) -> str:
        if execution_mode == "stream":
            if self._stream_path != path:
                self._stream_initialized = False
                self._stream_path = path

            if write_mode == "overwrite":
                file_mode = "w" if not self._stream_initialized else "a"
            else:
                file_mode = "a"

            self._stream_initialized = True
            return file_mode

        return "a" if write_mode == "append" else "w"

    def _ensure_parent_dir(self, path: str) -> None:
        parent = Path(path).parent
        if str(parent) not in {"", "."}:
            parent.mkdir(parents=True, exist_ok=True)
